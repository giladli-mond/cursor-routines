...
| Tool | Purpose |
|------|---------|
| `gmail_search` | Search inbox by query (same syntax as Gmail search box) |
| `gmail_get` | Get full content of a specific email by message ID |
...
Where `<messageId>` is the ID returned by `gmail_search` or `gmail_get`.
...
- **Urgent:** from known stakeholders (monday.com leadership, external partners, direct reports), contains action words ("please review", "deadline", "ASAP", "urgent"), flagged/starred
...
- Flag anything from monday.com leadership or external partners as high priority
