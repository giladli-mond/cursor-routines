...
| Tool | Purpose |
|------|---------|
| `calendar_listEvents` | List today's calendar events with attendees and details |
| `calendar_getEvent` | Get full details for a specific event (if needed) |
...
| **Customer meeting** | Any attendee with a non-`monday.com` email domain (excluding known partners, recruiters, tools like Zoom/Google) |

### Step 3 -- Identify customer meetings

A meeting is a **customer meeting** if:
- At least one attendee has an email domain that is NOT `monday.com`, `google.com`, `zoom.us`, `calendly.com`, `microsoft.com`, or other common tool/service domains
