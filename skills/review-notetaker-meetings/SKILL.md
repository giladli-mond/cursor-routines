...
description: Guide for using the monday AI notetaker via MCP. Covers what the tool can do, how to call it, and how to synthesise meeting data into actionable outputs.
---
...
The monday AI notetaker records meetings, generates transcripts, and extracts summaries, topics, and action items automatically. Access it via the `get_notetaker_meetings` tool on the `user-monday-api-mcp` server.
...
Use `CallMcpTool` with server `user-monday-api-mcp` and tool `get_notetaker_meetings`.
...
1. Read the tool descriptor for your meeting notetaker MCP (check your MCP tools folder for the JSON descriptor file).
