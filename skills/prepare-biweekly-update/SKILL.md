...
**Folder URL:** https://drive.google.com/drive/folders/1Iu0VCMC7-89BNtpVaszQqU7q8DYozSYi
**Folder ID:** `1Iu0VCMC7-89BNtpVaszQqU7q8DYozSYi`

This folder contains one Google Slides deck per bi-weekly session as a historical archive.

## Slack Channel

**Target channel:** `#monday-campaigns` (ID: `C08H4KT85DZ`)
...
| Tool | Purpose |
|------|---------|
| `drive_search` | Find the latest presentation in the bi-weekly folder |
| `slides_getText` | Extract text content from the most recent deck |
| `slides_getMetadata` | Get metadata (title, slide count, last modified) |

### Kremer MCP (`user-kremer-mcp`)

| Tool | Purpose |
|------|---------|
| `execute-sql-query` | Query current metrics from Snowflake |
| `data-expert-agent` | Explore metrics when the data guide doesn't cover a specific query |
...
| `slack_send_message_draft` | Draft the prep message to #monday-campaigns |
...
Tool: drive_search
Arguments:
  query: "https://drive.google.com/drive/folders/1Iu0VCMC7-89BNtpVaszQqU7q8DYozSYi"
```
...
| Metric | Source |
|--------|--------|
| ARR | Kremer (`execute-sql-query`) |
| Paying accounts | Kremer |
| DAU | Kremer |
| Campaigns sent (last 14 days) | Kremer |
...
Also search in other campaigns channels:
...
Arguments:
  ...
  sort_dir: "desc"
  limit: 20
  context_channel_id: "C08H4KT85DZ"
```
...
• DAU: [current] ([+/- delta])
• Campaigns sent (14d): [current] ([+/- delta])
