...
| Tool | Purpose |
|------|---------|
| `calendar_listEvents` | List this week's calendar events with attendees and details |

### Monday MCP (`user-monday-api-mcp`)

| Tool | Purpose |
|------|---------|
| `get_notetaker_meetings` | Check what meetings are coming up this week (if data available) |
...
- **Detect customer meetings:** any meeting where at least one attendee has a non-`monday.com` email domain (excluding tool domains like `google.com`, `zoom.us`, `calendly.com`, `microsoft.com`)
