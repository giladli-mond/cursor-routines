...
description: Queries main monday campaigns metrics via Kremer MCP and surfaces highlights, anomalies, and trends. Use as part of the daily start-of-day routine.
---
...
| Tool | Purpose |
|------|---------|
| `execute-sql-query` | Run SQL against Snowflake for specific metrics |
| `data-expert-agent` | AI agent for open-ended data exploration |
| `dbt-query` | Explore table schemas and column descriptions |
...
Use `execute-sql-query` for well-defined queries. Use `data-expert-agent` if you need to explore or the data guide doesn't cover a specific metric.
