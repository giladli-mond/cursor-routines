...
Review campaigns-related Slack messages from today. Produce a summary with highlights, key decisions, and anything needing follow-up.
...
Check the monday AI notetaker for today's meetings. Summarise what was discussed, extract action items, and flag unresolved topics.
...
### 4. Review Todoist Tasks

**Skill:** `skills/review-todoist-tasks/SKILL.md`
**State field:** `daily_end.review_todoist_tasks`

Fetch all Todoist tasks due today and append a "Today's Tasks" section to the EOD summary HTML. Each task shows a Postpone button (to defer to tomorrow) and an Open in Todoist link. After reviewing, the user can ask the agent to postpone marked tasks.
