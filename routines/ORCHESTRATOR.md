...
- **`/goodnight`** → Read `routines/daily/end-of-day.md` and run any skills whose state field is still `false`. Gmail EOD (step 3) appends email highlights to the EOD summary HTML. Todoist review (step 4) appends today's tasks with Postpone and Open in Todoist buttons. If `daily_end.flag == 1`, tell the user end-of-day routines are already done.
...
|-------------|------|-------|-----------|
| `review_ask_campaigns` | Review Ask-Campaigns channel (7-day) | Product Ops agent | `Agents/product-ops-agent.md` |
| `review_campaigns_churns` | Review Campaigns Churns channel (7-day) | Product Ops agent | `Agents/product-ops-agent.md` |
...
|------|-----------------------|-----------|
| `review_campaigns_churns` | Any churned accounts | Create a Todoist task for today: "Send personalised churn feedback emails to [list of account names]". Use the `write-churn-feedback-email` skill (`~/.cursor/skills/write-churn-feedback-email/SKILL.md`) to draft each email. |
...
| 4 | Review Todoist Tasks | `skills/review-todoist-tasks/SKILL.md` | `review_todoist_tasks` |
...
/goodmorning — Start of day (Slack DMs, Campaigns channels, summary + tasks, metrics, competitor intel on Mondays, Gmail, calendar + customer prep)
/goodnight — End of day (summarise campaigns day, review notetaker meetings, Gmail highlights, Todoist task review)
...
/mondaytasks — Monday recurring tasks (ask-campaigns, churns, sync skills, check Google MCP)
